...
def run_all_tests():
    ...
    # With biased twin, mean might be pulled toward 0, but shouldn't explode
    ...
    results.append({
        ...
        'threshold': {'no_divergence': True},
        'passed': abs(est_mean) < 10 and est_var > 0,  # Not diverged
    })
    ...
    # Plus exploitation once means update. Threshold: > random chance (25%)
    results.append({
        'test_id': 'R-L4-05',
        'name': 'Non-Stationary Rewards',
        'type': 'robust',
        'metrics': {'best_arm_rate_late': float(best_arm_rate)},
        'threshold': {'min_rate': 0.15},
        'passed': best_arm_rate > 0.15,  # Better than pure random 1/4 minus margin
    })
