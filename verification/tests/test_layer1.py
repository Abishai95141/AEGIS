...
def run_all_tests():
    ...
    # AND proxy results should be semantically consistent
    all_processed = True
    proxy_results_valid = True
    proxy_trace = []  # Log for auditability
    for text in ambiguous_proxy_texts:
        try:
            ...
            if not isinstance(result['w_proxy'], (float, int)):
                proxy_results_valid = False
            # Semantic consistency: w_proxy must be in [0, 1]
            if not (0.0 <= result['w_proxy'] <= 1.0):
                proxy_results_valid = False
            proxy_trace.append({
                'text': text,
                'z_proxy': bool(result['z_proxy']),
                'w_proxy': float(result['w_proxy']),
                'concepts': result['concept_ids'],
                'entropy': float(result['entropy']),
            })
        except Exception:
            all_processed = False

    # At least one ambiguous text should extract some concepts
    any_concepts_found = any(len(p['concepts']) > 0 for p in proxy_trace)
    ...
    results.append({
        ...
        'metrics': {
            ...
            'valid_proxy_types': proxy_results_valid,
            'any_concepts_found': any_concepts_found,
            'proxy_trace': proxy_trace,
        },
        'threshold': {'processes_without_crash': True, 'concepts_extracted': True},
        'passed': all_processed and proxy_results_valid and any_concepts_found,
    })
