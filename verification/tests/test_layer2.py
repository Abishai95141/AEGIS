...
def run_all_tests():
    ...
    # INTERMEDIATE STATE PROBE: verify Q adaptation happened via
    # innovation-based mechanism, not a single hardcoded jump.
    # Check that adaptation occurred across multiple steps.
    q_ratio_history = ukf.q_ratio_history
    adaptation_steps = len(q_ratio_history)
    q_diag_full = np.diag(ukf.Q).tolist()
    innovation_count = len(ukf.innovation_history)

    # Q ratio should have changed over multiple steps (not a single jump)
    multi_step_adaptation = adaptation_steps >= 3
