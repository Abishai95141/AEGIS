...
def run_all_tests():
    ...
    # Pass simulator directly for true closed-loop control
    pipeline = AEGISPipeline()
    trace_df = pipeline.run_simulation(sim, n_days=SIM_DAYS, train_nn_every=500)

    all_layers_active = (
        len(trace_df) > 0 and
        'L1_concepts' in trace_df.columns and
        'L2_predicted' in trace_df.columns and
        'L4_action' in trace_df.columns and
        'L5_final_action' in trace_df.columns
    )

    n_days_covered = trace_df['day'].nunique() if 'day' in trace_df.columns else 1
    expected_steps = SIM_DAYS * int(24 * 60 / 5)
    completion_rate = len(trace_df) / expected_steps
    ...
    # Use controlled glucose (reflecting pipeline's insulin decisions)
    metrics = pipeline.get_clinical_metrics()
    ...
    for pid in range(5):
        ...
        ppipe = AEGISPipeline()
        ptrace = ppipe.run_simulation(psim, n_days=3, train_nn_every=500)

        pmetrics = ppipe.get_clinical_metrics()
    ...
    # Phase 2: Force hypoglycemic event (glucose < 54 → EMERGENCY)
    for i in range(10):
        rec_pipe.step(45 + i * 3, 0, 0, "Feeling shaky")
    ...
    # After recovery, system should operate safely
    # STL_BLOCKED is VALID — the safety layer may correctly reduce high L4 doses
    post_recovery_tiers = [t['L5_tier'] for t in rec_pipe.trace[-10:]]
    recovered = all(t in ('OK', 'SELDONIAN_REDUCED', 'STL_BLOCKED') for t in post_recovery_tiers)
    ...
    full_trace = full_pipe.run_simulation(sim6, n_days=2, train_nn_every=500)
    ...
    l4_unique_actions = full_trace['L4_action'].nunique() if 'L4_action' in full_trace.columns else 0
