...
class HovorkaPatientSimulator:
    ...
    def _get_base_params(self, patient_type):
        ...
        if patient_type == 'child':
            return {
                ...
                'tmaxG': 40.0, 'AG': 0.8,
                'tmaxI': 55.0,
                'kb1': 0.0000034, 'kb2': 0.000056, 'kb3': 0.000024,
            }
    ...
    def _init_state(self):
        """Initialize to steady-state using analytical solutions."""
        p = self.params
        
        # Basal plasma insulin (mU/L) — typically ~10 mU/L for fasting
        I_basal = 10.0
        
        # Steady-state insulin action: dx/dt = 0 → x_ss = kb * I / ka
        x1_ss = p['kb1'] * I_basal / p['ka1']
        x2_ss = p['kb2'] * I_basal / p['ka2']
        x3_ss = p['kb3'] * I_basal / p['ka3']
        
        self.state = np.array([
            0.0,     # S1: subcut insulin 1
            0.0,     # S2: subcut insulin 2
            I_basal, # I: plasma insulin (mU/L)
            x1_ss,   # x1: insulin action on glucose transport
            x2_ss,   # x2: insulin action on disposal
            x3_ss,   # x3: insulin action on EGP
            self.Gb * p['Vg'] * self.BW / 18.0,  # Q1: glucose mass
            self.Gb * p['Vg'] * self.BW * 0.5 / 18.0,  # Q2
    ...
    def _rk4_step(self, state, insulin_input, carb_input, dt_min):
        """4th-order Runge-Kutta integration with 1-min substeps for stability."""
        num_substeps = int(dt_min)
        dt = 1.0  # 1 minute

        # Spread the inputs evenly over the substeps
        # insulin_input is total Units (U). ODE expects mU/min.
        # carb_input is total grams. ODE expects g/min.
        ins_sub = (insulin_input * 1000.0) / dt_min if dt_min > 0 else 0
        carb_sub = carb_input / dt_min if dt_min > 0 else 0

        current_state = np.copy(state)
        for _ in range(num_substeps):
            k1 = self._hovorka_ode(current_state, ins_sub, carb_sub)
            k2 = self._hovorka_ode(current_state + 0.5 * dt * k1, ins_sub, carb_sub)
            k3 = self._hovorka_ode(current_state + 0.5 * dt * k2, ins_sub, carb_sub)
            k4 = self._hovorka_ode(current_state + dt * k3, ins_sub, carb_sub)
        ...
        Vg = self.params['Vg']
        BW = self.BW
        q1_min = 40.0 * Vg * BW / 18.0
        q1_max = 400.0 * Vg * BW / 18.0
        current_state[6] = np.clip(current_state[6], q1_min, q1_max)
    ...
    # ================================================================
    # Interactive stepping API (for true closed-loop simulation)
    # ================================================================

    def init_scenario(self, n_days=7, dt_min=5):
        """
        Pre-generate the environment scenario (meals, exercise) for n_days
        WITHOUT computing insulin. Insulin comes from the pipeline.

        Must be called before using step().

        Returns the total number of steps.
        """
        self._init_state()
        self._scenario_dt = dt_min
        self._scenario_days = n_days
        self._steps_per_day = int(24 * 60 / dt_min)
        self._total_steps = self._steps_per_day * n_days
        self._current_step = 0
        self._start_time = datetime(2025, 1, 1, 0, 0)

        # Pre-generate meal and exercise schedules (patient behavior)
        self._meal_schedules = {}
        self._exercise_schedules = {}
        self._basal_rates = {}
        for day in range(n_days):
            self._meal_schedules[day] = self._generate_meal_schedule(day)
            self._exercise_schedules[day] = self._generate_exercise_schedule(day)
            basal = 0.8 + 0.3 * self.rng.randn()
            self._basal_rates[day] = max(0.2, min(2.0, basal))

        return self._total_steps

    def get_scenario(self, step_idx=None):
        """
        Get the environment state for the current (or given) timestep.

        Returns dict with:
            day, hour, timestamp, carb_input, meal_carbs, basal_rate,
            notes, stress_level, proxy_z/w, exercise_state
        """
        if step_idx is None:
            step_idx = self._current_step

        dt_min = self._scenario_dt
        day = step_idx // self._steps_per_day
        step_in_day = step_idx % self._steps_per_day
        hour_of_day = (step_in_day * dt_min / 60.0) % 24.0
        time_minutes = step_idx * dt_min
        timestamp = self._start_time + timedelta(minutes=time_minutes)

        # Update latent variables
        self._update_latent_variables(hour_of_day, day)

        # Determine carb input from pre-generated meal schedule
        meals = self._meal_schedules.get(day, [])
        carb_input = 0.0
        meal_carbs = 0.0
        ...
        # Exercise
        exercises = self._exercise_schedules.get(day, [])
        ...
        # Generate text and proxies
        glucose = self._get_glucose_mg_dl()
        notes = self._generate_text(glucose, hour_of_day, meal_carbs,
                                     exercise_just_started)
        ...
        return {
            ...
            'meal_carbs': float(meal_carbs),
            'basal_rate': float(self._basal_rates.get(day, 0.8)),
            'notes': notes,
    ...
    def sim_step(self, insulin_input, carb_input=None, dt_min=None):
        """
        Advance the Hovorka ODE one timestep with the given insulin dose.

        This is the core closed-loop interface: the PIPELINE decides
        the insulin dose, and this method advances the physiology.

        Args:
            insulin_input: Total insulin for this step (bolus + basal, in units)
            carb_input: Carb input (grams). If None, uses pre-generated scenario.
            dt_min: Timestep in minutes. If None, uses scenario default.

        Returns: glucose_mg_dl after this step
        """
        if dt_min is None:
            dt_min = getattr(self, '_scenario_dt', 5)

        # If carb_input not provided, use the scenario's value
        if carb_input is None and hasattr(self, '_current_step'):
            scenario = self.get_scenario()
            carb_input = scenario['carb_input']

        carb_input = carb_input or 0.0
        ...
        return self._get_glucose_mg_dl()
