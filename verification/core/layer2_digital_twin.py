...
class AdaptiveConstrainedUKF:
    ...
    def _adapt_Q(self, innovation, predicted_var, K):
        """
        Innovation-based process noise covariance adaptation.

        Implements the paper's exact equation (Eq. 4):
            Q_{k+1} = Q_k + α · K · (ε_k · ε_kᵀ − S_k) · Kᵀ

        Where:
            ε_k = measurement innovation (scalar → 1×1 matrix)
            S_k = predicted innovation covariance (Pzz from update step)
            K   = Kalman gain (n×1 matrix)
            α   = adaptation rate (L2_UKF_Q_ADAPT_RATE, default 0.1)

        This replaces the previous bounded-multiplier approach which
        was identified as a shallow proxy (capped inflation at 1.5x,
        used 0.99 shrink, diagonal-only updates).

        Post-update: eigenvalue clamping ensures Q remains positive
        semi-definite. No hardcoded multiplier bounds.

        PROHIBITED (per System Instructions):
            - Bounded multipliers (e.g., min(inflate, 1.5))
            - Scalar norm instead of full outer product
            - Diagonal-only updates
        """
        window = 10
        if len(self.innovation_history) >= window:
            # --- Eq. 4: Q_{k+1} = Q_k + α · K · (ε·εᵀ − S) · Kᵀ ---

            # ε_k is the scalar innovation → treat as 1×1 matrix
            # Use the rolling empirical innovation outer product for stability
            recent = np.array(self.innovation_history[-window:])
            empirical_eps_epsT = np.mean(recent**2)  # E[ε·εᵀ] scalar

            # S_k = predicted innovation covariance (scalar for 1D measurement)
            S_k = predicted_var

            # Innovation-covariance residual: (ε·εᵀ − S)
            innovation_residual = empirical_eps_epsT - S_k

            # K is (n×1), so K·(scalar)·Kᵀ produces an (n×n) matrix
            # This is the full outer-product update, NOT a diagonal scaling
            K_col = K.reshape(-1, 1) if K.ndim == 1 else K
            delta_Q = L2_UKF_Q_ADAPT_RATE * innovation_residual * (K_col @ K_col.T)

            # Update Q with the full matrix correction
            Q_new = self.Q + delta_Q

            # Ensure Q remains positive semi-definite via eigenvalue clamping
            # (NOT via bounded multipliers)
            eigenvalues, eigenvectors = np.linalg.eigh(Q_new)

            # Clamp eigenvalues: floor at Q_base minimum eigenvalue,
            # ceiling at Q_base maximum eigenvalue × MAX_RATIO
            q_base_eigs = np.linalg.eigvalsh(self.Q_base)
            min_eig = q_base_eigs.min() * 0.1  # Allow shrinking to 10% of base
            max_eig = q_base_eigs.max() * L2_UKF_Q_MAX_RATIO

            eigenvalues_clamped = np.clip(eigenvalues, min_eig, max_eig)
            self.Q = eigenvectors @ np.diag(eigenvalues_clamped) @ eigenvectors.T
...
class DigitalTwin:
    ...
    def __init__(self):
        ...
        # Phase 3: Thin attributes for BergmanParameterAdapter
        self.innovation_var_ema = 1.0  # EMA of innovation variance
        self._dt = 5.0                 # Timestep in minutes
    ...
    def predict_trajectory(self, insulin_schedule, carb_schedule, n_steps, dt=5.0):
        """
        Forward-simulate n_steps using the Bergman ODE from current state.

        Does NOT modify internal filter state — uses a temporary copy.
        This is the interface used by MPC to evaluate candidate doses.

        Innovation-based bias correction: uses the mean of recent UKF
        innovations (actual - predicted) to correct for systematic
        Bergman model bias. Finding 8 showed -131.7 mg/dL mean signed
        error — Bergman under-predicts glucose in the hyperglycemic range.

        Args:
            insulin_schedule: list/array of insulin doses per step (length n_steps)
            carb_schedule: list/array of carb inputs per step (length n_steps)
            n_steps: horizon length
            dt: timestep in minutes (default 5.0)

        Returns:
            glucose_trajectory: array of predicted glucose (length n_steps)
            uncertainty_trajectory: array of predicted variance (length n_steps)
        """
        # Take a snapshot of current state (do not modify filter)
        state = self.get_state().copy()

        # Innovation-based bias correction (Step 2b):
        # self.residuals stores (actual - predicted) at each update() call.
        # Mean of last 20 innovations estimates systematic model bias.
        # Applied to forward predictions only — UKF state is already corrected.
        bias = 0.0
        if len(self.residuals) >= 10:
            recent = self.residuals[-20:]  # Last 20 (or fewer if <20 available)
            bias = float(np.mean(recent))

        # Apply bias to initial state for forward prediction
        state[0] += bias

        # Initial glucose uncertainty from current filter
        if self.active_filter == 'UKF':
            glucose_var_0 = self.ukf.P[0, 0]
            q_glucose = self.ukf.Q[0, 0]
        else:
            _, glucose_var_0 = self.rbpf.get_estimate()
            q_glucose = self.rbpf.Q_nl

        glucose_trajectory = np.zeros(n_steps)
        uncertainty_trajectory = np.zeros(n_steps)

        for k in range(n_steps):
            ins = float(insulin_schedule[k]) if k < len(insulin_schedule) else 0.0
            carb = float(carb_schedule[k]) if k < len(carb_schedule) else 0.0

            # Advance Bergman ODE one step
            state = self.bergman.rk4_step(state, ins, carb, dt)

            # Apply neural residual correction if trained
            if self._nn_trained:
                import torch
                with torch.no_grad():
                    inp = torch.FloatTensor([state[0], state[1], state[2], ins, carb])
                    residual = self.neural_residual(inp).numpy()
                    state = state + residual * 0.1

            # Re-apply bias correction at each step (bias is for open-loop drift)
            state[0] += bias * 0.1  # Fractional application per step to avoid overcorrection
            ...
            glucose_trajectory[k] = state[0]
            # Uncertainty grows linearly with each step (open-loop propagation)
            uncertainty_trajectory[k] = glucose_var_0 + (k + 1) * q_glucose

        return glucose_trajectory, uncertainty_trajectory
    ...
    # ── Phase 3: Thin property accessors for BergmanParameterAdapter ──

    @property
    def last_innovation(self):
        """Last UKF innovation (actual - predicted). Read-only."""
        return self.residuals[-1] if self.residuals else 0.0

    @property
    def glucose_basal(self):
        """Bergman basal glucose Gb. Read-only."""
        return self.bergman.Gb

    @property
    def R_noise(self):
        """Measurement noise variance (scalar). Read-only."""
        return self.ukf.R[0, 0]

    @property
    def state(self):
        """Current state estimate array [G, X, I]. Read-only."""
        return self.get_state()

    @property
    def dt(self):
        """Integration timestep in minutes. Read-only."""
        return self._dt

    def set_p1(self, value):
        """Thin setter — writes p1 to the Bergman ODE instance."""
        self.bergman.p1 = value


# ──────────────────────────────────────────────────────────
# Phase 3: Bergman Parameter Adapter (p1-Only Augmented UKF)
# ──────────────────────────────────────────────────────────
...
class BergmanParameterAdapter:
    ...
    Q_P1_ACTIVE = 1e-10   # when identifiability gate is open (slow adaptation)
    Q_P1_FROZEN = 0.0     # when gate is closed (flat glucose)

    # Identifiability gate threshold:
    # The Bergman-Hovorka mismatch produces ~15 mg/dL baseline innovations.
    # The gate closes when innovation variance drops below this threshold,
    # which only happens during truly flat glucose (near target, no meals).
    INNOVATION_VAR_GATE = 50.0  # mg²/dL²

    # Warm-up: freeze p1 for the first N steps while UKF settles
    WARMUP_STEPS = 20

    def __init__(self, digital_twin, p1_init=0.028):
        self.twin = digital_twin
        self.p1_est = float(p1_init)
        self.p1_var = 1e-4    # initial uncertainty on p1

        # Bounds violation tracking
        self._consecutive_at_bound = 0
        self._bound_warnings = 0
        self._step_count = 0

    def _identifiability_gate_open(self):
        """Returns True if innovation variance exceeds the threshold."""
        return self.twin.innovation_var_ema >= self.INNOVATION_VAR_GATE

    def update(self, glucose_obs):
        """
        ...
        # Update innovation_var_ema on the twin from the latest innovation
        innovation = self.twin.last_innovation
        LAMBDA_EMA = 0.95  # smoothing factor for innovation variance EMA
        self.twin.innovation_var_ema = (
            LAMBDA_EMA * self.twin.innovation_var_ema
            + (1 - LAMBDA_EMA) * innovation ** 2
        )

        # Warm-up: freeze p1 while UKF settles
        ...
        self.twin.set_p1(p1_pred)   # thin setter
        ...
        K_p1 = p1_var_pred * H_p1 / max(S, 1e-12)     # Kalman gain for p1

        p1_delta = K_p1 * innovation
        
        # Rate limit: max change per step to prevent cumulative drift
        # from persistent structural model mismatch innovations
        MAX_P1_STEP = 1e-5
        p1_delta = np.clip(p1_delta, -MAX_P1_STEP, MAX_P1_STEP)
        
        p1_updated = p1_pred + p1_delta
        ...
        # Bounds violation tracking
        if self.p1_est == self.P1_MIN or self.p1_est == self.P1_MAX:
            self._consecutive_at_bound += 1
            if self._consecutive_at_bound > 10:
                self._bound_warnings += 1
                # Warning logged: p1 has been at a physiological bound for >10 steps
        else:
            self._consecutive_at_bound = 0

        # Apply to twin so next ODE step uses this p1
        self.twin.set_p1(self.p1_est)
    ...
    def get_stats(self):
        ...
        return {
            'p1_est': self.p1_est,
            'p1_var': self.p1_var,
            'gate_open': self._identifiability_gate_open(),
            'innovation_var_ema': self.twin.innovation_var_ema,
            'bound_warnings': self._bound_warnings,
            'consecutive_at_bound': self._consecutive_at_bound,
        }
