...
- Harmonic G-estimation with Fourier basis (K=3) — Eq. III-C
...
Note: h*(W) bridge function is ESTIMATED via KernelRidge, never assumed.
      Bridge function h*(W)=0 or h*(W)=1 are never hardcoded.
"""
...
class HarmonicGEstimator:
    ...
    Estimating equation (doubly-robust form):
        Σ_t [Y_{t+1} - μ̂(S_t) - τ(t;ψ) A_t] · (A_t - p_t(S_t)) · h(t) = 0

    Variance estimation: sandwich estimator (HC0).

    NOT a shallow proxy: uses OLS with full W matrix and sandwich
    variance, not a scalar approximation.
    """
...
class AIPWEstimator:
    ...
    Paper Eq. III-D:
        τ̂_AIPW = n⁻¹ Σ_i [(A·Y - (A-e)·μ̂₁)/e - ((1-A)·Y + (A-e)·μ̂₀)/(1-e)]

    NOT a shallow proxy: both outcome and propensity models are fit using
    real sklearn estimators, not hardcoded values.
    """
...
class ProximalGEstimator:
    ...
    def estimate_bridge_function(self, Z, W, S):
        ...
        Hyperparameter selection:
            gamma: median heuristic — γ = 1 / median(||x_i - x_j||²)
            alpha: GCV over {0.001, 0.01, 0.1, 1.0}

        Audit: selected hyperparameters logged in self._bridge_params
        """
        if S.ndim == 1:
            S = S.reshape(-1, 1)
        ...
        # --- Median heuristic for γ (replaces fixed gamma=0.1) ---
        from sklearn.metrics.pairwise import pairwise_distances
        dists = pairwise_distances(X_bridge, metric='sqeuclidean')
        # Take median of non-zero pairwise distances
        triu_idx = np.triu_indices_from(dists, k=1)
        median_dist = np.median(dists[triu_idx])
        gamma = 1.0 / max(median_dist, 1e-10)

        # --- GCV for α (replaces fixed alpha=0.01) ---
        # Grid search over alpha candidates
        best_alpha = self.alpha
        best_score = -np.inf
        n = len(X_bridge)

        if n >= 10:  # Only do GCV if we have enough data
            from sklearn.metrics.pairwise import rbf_kernel
            K = rbf_kernel(X_bridge, gamma=gamma)
            for alpha_candidate in [0.001, 0.01, 0.1, 1.0]:
                # GCV score: ||y - K(K + αI)⁻¹y||² / (1 - tr(H)/n)²
                # where H = K(K + αI)⁻¹
                try:
                    KaI = K + alpha_candidate * np.eye(n)
                    KaI_inv = np.linalg.inv(KaI)
                    H = K @ KaI_inv
                    y_hat = H @ W.ravel()
                    residuals = W.ravel() - y_hat
                    leverage = np.diag(H)
                    # LOO-CV via shortcut: e_i/(1 - h_ii)
                    loo_residuals = residuals / (1 - leverage + 1e-10)
                    gcv_score = -np.mean(loo_residuals ** 2)
        ...
        self.bridge_model = KernelRidge(
            kernel=self.kernel, alpha=best_alpha, gamma=gamma
        )
        ...
        self._bridge_params = {
            'gamma': float(gamma),
            'gamma_source': 'median_heuristic',
            'alpha': float(best_alpha),
            'alpha_source': 'gcv' if n >= 10 else 'default',
...
class CausalInferenceEngine:
    ...
    def process(self, z=None, w=None, treatment=None, outcome=None,
                glucose=None, insulin=None, carbs=None):
        """
        Pipeline-compatible adapter for single-step causal processing.

        Accumulates observations and returns the latest causal effect
        estimate. Batch estimation occurs every 50 steps when sufficient
        data has been collected.

        Args:
            z: treatment-confounder proxy (from Layer 1)
            w: outcome-confounder proxy (from Layer 1)
            treatment: 'bolus' or 'no_bolus'
            outcome: reward signal
            glucose, insulin, carbs: raw measurements

        Returns dict with:
            causal_effect, randomization_prob, method
        """
        ...
        A = 1 if treatment == 'bolus' else 0
        t = float(len(self._obs_buffer['t']) % 288) * 5.0 / 60.0  # 5-min steps → hours mod 24

        self._obs_buffer['t'].append(t)
        self._obs_buffer['A'].append(A)
        self._obs_buffer['Y'].append(float(outcome) if outcome is not None else 0.0)
        self._obs_buffer['S'].append([
            float(glucose) if glucose else 120.0,
            float(insulin) if insulin else 0.0,
            float(carbs) if carbs else 0.0,
        ])
        self._obs_buffer['Z'].append(float(z) if z is not None else 0.0)
        self._obs_buffer['W'].append(float(w) if w is not None else 0.0)

        n = len(self._obs_buffer['t'])
        result = {
            'causal_effect': 0.0,
            'randomization_prob': L3_RANDOMIZATION_RANGE[0],
            'method': 'accumulating',
        }

        # Batch estimate every 50 steps when we have enough data
        if n >= 50 and n % 50 == 0:
            try:
                t_arr = np.array(self._obs_buffer['t'])
                A_arr = np.array(self._obs_buffer['A'])
                Y_arr = np.array(self._obs_buffer['Y'])
                S_arr = np.array(self._obs_buffer['S'])
                Z_arr = np.array(self._obs_buffer['Z'])
                W_arr = np.array(self._obs_buffer['W'])

                estimates = self.estimate_treatment_effect(
                    t_arr, A_arr, Y_arr, S_arr,
                    Z=Z_arr, W=W_arr
                )

                ce = estimates.get('aipw', {}).get('ate', 0.0)
                result = {
                    'causal_effect': float(ce),
                    'randomization_prob': np.clip(
                        L3_RANDOMIZATION_RANGE[0] + abs(ce) * 0.1,
                        L3_RANDOMIZATION_RANGE[0],
                        L3_RANDOMIZATION_RANGE[1]
                    ),
                    'method': 'aipw',
                    'estimates': estimates,
                }
            except Exception:
                pass  # Keep accumulating if estimation fails
